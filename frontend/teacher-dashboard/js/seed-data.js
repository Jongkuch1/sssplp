...
async function seedData() {
    ...
    if (!localStorage.getItem('users')) {
        const users = [
            { id: 1, name: 'John Doe', email: 'student@ssplp.org', password: '123', role: 'student', grade: 10, status: 'active', createdAt: Date.now() - 86400000 * 30, lastActive: Date.now() - 86400000 * 1 },
            { id: 2, name: 'Jane Smith', email: 'student2@ssplp.org', password: '123', role: 'student', grade: 11, status: 'active', createdAt: Date.now() - 86400000 * 25, lastActive: Date.now() - 86400000 * 2 },
            { id: 3, name: 'Sarah Williams', email: 'teacher@ssplp.org', password: '123', role: 'teacher', status: 'active', createdAt: Date.now() - 86400000 * 60 },
            { id: 4, name: 'Admin User', email: 'admin@ssplp.org', password: '123', role: 'admin', status: 'active', createdAt: Date.now() - 86400000 * 90 }
        ];
    ...
    // Courses
    const courses = [
        { id: 1, title: 'Mathematics Grade 10', subject: 'Mathematics', teacherId: 3, students: [1, 2], createdAt: Date.now() - 86400000 * 30 },
        { id: 2, title: 'Physics Fundamentals', subject: 'Physics', teacherId: 3, students: [1], createdAt: Date.now() - 86400000 * 25 },
        { id: 3, title: 'English Literature', subject: 'English', teacherId: 3, students: [2], createdAt: Date.now() - 86400000 * 20 }
    ];
    localStorage.setItem('courses', JSON.stringify(courses));
    
    // Assignments
    const assignments = [
        { id: 1, title: 'Algebra Problem Set', subject: 'Mathematics', teacherId: 3, courseId: 1, dueDate: new Date(Date.now() + 86400000 * 7).toISOString(), totalPoints: 100, createdAt: Date.now() - 86400000 * 5 },
        { id: 2, title: 'Physics Lab Report', subject: 'Physics', teacherId: 3, courseId: 2, dueDate: new Date(Date.now() + 86400000 * 10).toISOString(), totalPoints: 50, createdAt: Date.now() - 86400000 * 3 },
        { id: 3, title: 'Essay on Shakespeare', subject: 'English', teacherId: 3, courseId: 3, dueDate: new Date(Date.now() + 86400000 * 14).toISOString(), totalPoints: 100, createdAt: Date.now() - 86400000 * 2 }
    ];
    localStorage.setItem('assignments', JSON.stringify(assignments));
    
    // Submissions
    const submissions = [
        { id: 1, assignmentId: 1, studentId: 1, submittedAt: Date.now() - 86400000 * 2, status: 'graded', grade: 85 },
        { id: 2, assignmentId: 1, studentId: 2, submittedAt: Date.now() - 86400000 * 1, status: 'graded', grade: 92 },
        { id: 3, assignmentId: 2, studentId: 1, submittedAt: Date.now() - 86400000 * 1, status: 'pending' }
    ];
    ...
    console.log('✅ Data seeded: 2 students, 1 teacher, 1 admin, 6 quiz attempts, 3 courses, 3 assignments, 3 submissions, 8 resources');
}
